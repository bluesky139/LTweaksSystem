...
import li.lingfeng.ltsystem.services.RemoteShell;
...
public class ComicScreenNoZipCache extends TweakBase {
    ...
    public static class ComicScreen extends TweakBase {
    ...
    @MethodsLoad(packages = PackageNames.ANDROID, prefs = R.string.key_comic_screen_no_zip_cache)
    public static class Android extends TweakBase {
        ...
        public void com_android_server_am_ActivityManagerService__handleAppDiedLocked__ProcessRecord_boolean_boolean(ILTweaks.MethodParam param) {
            param.before(() -> {
                ...
                if (PackageNames.COMIC_SCREEN.equals(processName)) {
                    Logger.w("Kill rclone from comic screen since app died.");
                    RemoteShell.su("ps -ef | grep \"rclone mount cloud0: \" | grep rclone_comic_screen | awk '{printf $2}' | xargs kill");
                }
            });
        }
    }
